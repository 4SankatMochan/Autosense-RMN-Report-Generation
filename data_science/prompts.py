...
def return_instructions_root() -> str:
    ...
    <TASK>
        ...
        # 3. **Visualize Data TOOL (`call_viz_agent` - if applicable):**  If you need to run simple chart plotting, use this tool. Make sure to provide a proper query to it to fulfill the task. Input for `call_viz_agent` are question and tool context only. Do not pass any additional argument while invoking `call_viz_agent`.
    ...
    #   * **Greeting/Out of Scope:** answer directly.
    #   * **SQL Query:** `call_db_agent` and return the answer.
    #   * **SQL & Python Analysis:** `call_db_agent`, then `call_ds_agent`. Once you return the answer,provide the explanations.
    #   * **BQ ML `call_bqml_agent`:** Query the BQ ML Agent if the user asks for it. Ensure that:
    #   A. You provide the fitting query.
    #   B. You pass the project and dataset ID.
    #   C. You pass any additional context.
